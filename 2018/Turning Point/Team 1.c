...
#pragma config(Motor,  port9,           armL,          tmotorVex393_MC29, PIDControl, reversed, encoderPort, I2C_2)
#pragma config(MotorPidSetting,  port9,  50, 64, 1000, 12, 10,  47, 0, 15)
...
void armLogic() {
	...
	if (toggleButtonSetter(&armPidLockToggler, vexRT[BTN_TOGGLE_LOCK_ARM_PID])) { // Runs if the value changes
		if (armPidState == ARM_PID_LOCKED) armPidState = ARM_PID_DISABLED; // Disable PID if currently locked
			...
			setMotorTarget(armL, armPid.target, 127, true);
		}
	}
...
task usercontrol()
{
	...
	while (true)
	{
		...
		if (nLCDButtons == 4) arm(0); //Disable PID when pressed.TEMPORARY
		else if (nLCDButtons == 2) setMotorTarget(armL, ARM_LOW_POST_HEIGHT, 100, true);
